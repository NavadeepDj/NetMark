    ...
    try {
      ...
      for (final key in keys) {
        ...
        if (key == 'faceEmbedding' && value is List<String>) {
          final embedding =
              value.map((e) => double.tryParse(e) ?? 0.0).toList();
          _logger.i(
              '   • $key: [${embedding.take(10).map((v) => v.toStringAsFixed(4)).join(', ')}...] (${embedding.length} total)');
        } else {
          _logger.i('   • $key: $value');
        }
      }
      ...
      if (faceEmbeddingList != null) {
        final embedding =
            faceEmbeddingList.map((e) => double.tryParse(e) ?? 0.0).toList();

        _logger.i('🎯 FACE EMBEDDING ANALYSIS:');
        _logger.i('   • Total dimensions: ${embedding.length}');
        _logger.i(
            '   • Min value: ${embedding.reduce((a, b) => a < b ? a : b).toStringAsFixed(4)}');
        _logger.i(
            '   • Max value: ${embedding.reduce((a, b) => a > b ? a : b).toStringAsFixed(4)}');
        _logger.i(
            '   • Average: ${(embedding.reduce((a, b) => a + b) / embedding.length).toStringAsFixed(4)}');
        _logger.i(
            '   • First 20 values: ${embedding.take(20).map((v) => v.toStringAsFixed(4)).join(', ')}');
